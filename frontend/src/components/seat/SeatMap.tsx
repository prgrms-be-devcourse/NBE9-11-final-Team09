...
// 25인승: A B | 통로 | C
function BUS25Layout({ seats, onSeatClick }: BusLayoutProps) {
            ...
            {rows.map((row) => (
                <div key={row} className="flex items-center gap-2">
                    <div className="flex gap-1">
                        {["A", "B"].map((col) =>
                            renderSeat(seats, `${row}${col}`, onSeatClick)
                        )}
                    </div>
...
// 45인승: A B | 통로 | C D
// 45인승: A B | 통로 | C D
function BUS45Layout({ seats, onSeatClick }: BusLayoutProps) {
    ...
    return (
        <div className="border-2 border-gray-300 rounded-xl p-6 bg-white">
            <h3 className="text-sm font-semibold mb-4">
                탑승 좌석 선택 (
                {busType === "BUS_25" ? "25인승 우등형" : "45인승 일반형"})
            </h3>
